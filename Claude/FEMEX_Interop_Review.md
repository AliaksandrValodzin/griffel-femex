...
> **Closed by `FEMEX_Identity.md`** (schema 1.3), taking the **SAF middle path** over the
> GUID-primary direction `FEMEX_Assessment.md` decision 3 had locked in. The observation above is what
> settled it: SAF had this exact decision with every vendor in the room and chose neither exclusively.
> So the integer id stays the in-file reference key — not one of `Bar.StartNodeId`, `Plate.NodeIds`,
> `AreaLoad.PlateId` or `Hinge.ElementId` changed, which is what makes the whole thing additive — and
> a new optional `Guid? Uid` sits beside it on all thirteen authored declaration sites through
> `IIdentified`, omitted from the JSON entirely when null. Uniqueness is model-wide, because that is
> what a GUID means and a receiver merging by uid does not care which list an object came from; a uid
> naming two objects and the nil uid written out are both errors. `Load` gained an `int Id` on the way,
> loads having been the only authored entity with no key at all, and a pre-1.3 file's loads are
> numbered 1..N in list order on read.
>
> Three things this section asked for were deliberately not done. **Names are a warning, not a
> requirement** — `Section.Name`, `SurfaceProperty.Name`, `Material.Name` and `LoadCase.Label` stay
> `string?`, and a blank or duplicated one raises the two-message warning `ValidateLoadCombinationUsage`
> already established for combination labels. That is the half-step: it tells an author what an
> exporter targeting Robot is about to have to invent, without invalidating a single existing file.
> **`Parent ID` was left out** — its documented use is reconstituting a curved member a receiver
> segmented, and FEMEX's curves-as-chords decision (§6) leaves nothing to reconstitute; the
> FEMEX-shaped cases (a plate cut into several, a bar split at an intersection) are real but have no
> reader yet. And **nothing mints a uid automatically**: `AssignMissingUids()` is a call the caller
> makes and it never overwrites, because auto-stamping on save would mean the same model built twice
> carried different uids and `Examples/Example1.femex` could never be regenerated byte-identically.
> Merging two FEMEX files is now *possible* and still not *implemented* — integer ids still collide,
> and a merge helper keying on uid is the obvious next step.
